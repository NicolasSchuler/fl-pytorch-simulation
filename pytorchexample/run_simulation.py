...
@click.option(
    ...
    "--num-clients",
    type=int,
    help="Number of clients participating in the federation",
)
...
@click.option(
    "--num-cpus-worker", type=int, help="Number of CPUs each worker can utilize"
)
@click.option(
    "--num-gpus-worker", type=float, help="Number of GPUs each worker can utilize"
)
@click.option(
    "--max-memory-total", type=int, help="Total maximum memory to be utilized"
)
@click.option(
    "--max-memory-worker",
    type=float,
    default=4,
    help="Specify maximum memory a worker uses (in GB)",
)
@click.option(
...
@click.option(
    "--ray-address", type=str, default="local", help="Address for ray cluster"
)
@click.option(
    "--ray-dashboard",
    is_flag=True,
    type=bool,
    default=False,
    help="Activate Ray Dashboard",
)
@click.option("--ray-dashboard-port", type=int, default=8265, help="Ray Dashboard port")
@click.option(
    "--ray-dashboard-host", type=str, default="127.0.0.1", help="Ray Dashboard host"
)
@click.option(
    "--ray-namespace",
    type=str,
    default="flwr-simulation",
    help="Namespace for Ray Cluster that is going to be used",
)
@click.option(
    "--run-id", type=str, help="ID for the current run. Defaults to generated UUID4"
)
@click.option(
    "--print-config",
    type=bool,
...
def simulation(**kwargs):
    ...
    try:
        ...
        click.echo(
            "Could not parse the toml file. Check if the toml file is valid!", err=True
        )
        exit(-1)
    ...
    if backend == "ray-task":
        ...
        if (
            click.get_current_context().get_parameter_source("ray_address")
            == ParameterSource.COMMANDLINE
        ):
            click.echo(
                "--ray-address is not supported in ProcessPool Engine -> ignoring",
                err=True,
            )
        if (
            click.get_current_context().get_parameter_source("ray_dashboard")
            == ParameterSource.COMMANDLINE
        ):
            click.echo(
                "--ray-dashboard is not supported in ProcessPool Engine -> ignoring",
                err=True,
            )
        if (
            click.get_current_context().get_parameter_source("ray_dashboard_port")
            == ParameterSource.COMMANDLINE
        ):
            click.echo(
                "--ray-dashboard-port is not supported in ProcessPool Engine -> ignoring",
                err=True,
            )
        if (
            click.get_current_context().get_parameter_source("ray_dashboard_host")
            == ParameterSource.COMMANDLINE
        ):
            click.echo(
                "--ray-dashboard-host is not supported in ProcessPool Engine -> ignoring",
                err=True,
            )
...
def validate_config(kwargs, pyproject):
    ...
    try:
        ...
        click.echo(
            "client_fn or server_fn could not be loaded. Check your paths", err=True
        )
    ...
    if kwargs["overwrite_app_config"]:
        try:
            pyproject["tool"]["flwr"]["app"]["config"].update(
                kwargs.pop("overwrite_app_config")
            )
        except Exception:
            click.echo(
                "Something went wrong while overwriting the app config!", err=True
            )
            exit(-1)
    ...
    if kwargs["fl_rounds"]:
        ...
        try:
            config["fl_rounds"] = pyproject["tool"]["flwr"]["app"]["config"][
                "num-server-rounds"
            ]
        except KeyError:
            click.echo(
    ...
    if kwargs["overwrite_federation_config"]:
        try:
            ...
            click.echo(
                "Something went wrong while overwriting the federation config", err=True
            )
            exit(-1)
    ...
    if kwargs["num_clients"]:
        ...
        try:
            config["num_clients"] = pyproject["tool"]["flwr"]["federations"][
                config["federation"]
    ...
    if kwargs["num_cpus_worker"]:
        ...
        if kwargs["backend"] == "proccess-pool":
            click.echo(
                "--num-cpus-worker option not supported when using ProcessPool Engine -> ignoring",
                err=True,
            )
    else:
        try:
            config["num_cpus_worker"] = pyproject["tool"]["flwr"]["federations"][
                config["federation"]
            ]["options"]["backend"]["client-resources"]["num-cpus"]
        except Exception:
            if kwargs["backend"] not in ["process-pool"]:
                click.echo(
                    "Could not determine number of cpus core each worker gets, will default to 1",
                    err=True,
                )
    ...
    if kwargs["num_gpus_worker"]:
        ...
        if kwargs["backend"] in ["process-pool", "process-pool-shm"]:
            click.echo(
                "--num-gpus-worker option not supported when using ProcessPool Engine -> ignoring",
                err=True,
            )
    else:
        try:
            config["num_gpus_worker"] = pyproject["tool"]["flwr"]["federation"][
                config["federation"]
            ]["options"]["backend"]["client-resources"]["num-gpus"]
        except Exception:
            if kwargs["backend"] not in ["process-pool"]:
                click.echo(
                    "Could not determine number of gpus each worker gets, will default to 0",
                    err=True,
                )
    ...
    if kwargs["num_cpus_total"]:
        ...
        if kwargs["backend"] in ["process-pool"]:
            click.echo(
                "--num-cpus-total option not supported when using ProcessPool Engine -> ignoring",
                err=True,
            )
    else:
        cpus = cpu_count()
        ...
        if kwargs["backend"] not in ["process-pool"]:
            click.echo(
                f"Total number of cpus not provided assuming all core are used ({cpus})"
            )
    ...
    if kwargs["num_gpus_total"]:
        ...
        if kwargs["backend"] in ["process-pool"]:
            click.echo(
                "--num-gpus-total option not supported when using ProcessPool Engine -> ignoring",
                err=True,
            )
    else:
        if kwargs["backend"] not in ["process-pool"]:
    ...
    if kwargs["max_memory_total"]:
        ...
        if kwargs["backend"] in ["process-pool"]:
            click.echo(
                "--max-memory-total option not supported when using ProcessPool Engine -> ignoring",
                err=True,
            )
    else:
        if kwargs["backend"] not in ["process-pool"]:
            click.echo(
                "Total maximum memory not provided, assuming 98% of total available memory"
            )
    ...
    if kwargs["num_workers"]:
        ...
        if kwargs["backend"] in ["process-pool"]:
            click.echo(
                "--num-workers option is required for process-pool backend", err=True
            )
            exit(-1)
    ...
    click.echo(
        f"According to the configuration, will use {num_workers} number of workers"
    )
