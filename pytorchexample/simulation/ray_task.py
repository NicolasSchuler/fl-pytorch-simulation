...
class RayTaskSimulator(Simulation):
    ...
    def run_loop(self):
        ...
        for server_round in range(1, self._fl_rounds + 1):
            ...
            fit_res, fit_failures = self._fit_clients(configure_fit_res, client_contexts, ray_fds)
            ...
            eval_res, eval_failures = self._evaluate_clients(configure_eval_res, client_contexts, ray_fds)
    ...
    def _evaluate_clients(self, configure_eval_res, contexts, fds: FederatedDataset):
        ...
        return self._call_clients(configure_eval_res, contexts, fds, ClientMethods.EVALUATE)

    def _call_clients(self, configure_res, contexts, fds: FederatedDataset, method: ClientMethods):
        with tqdm(total=self._num_clients, desc=f"Calling {method} on Clients", leave=True, unit="Client") as bar:
            ...
            for _k, g in itertools.groupby(configure_res, lambda cres: cres[1].parameters.tensors):
            ...
            for c, ins in configure_res:
                ...
                tasks[
                    ...
                    .options(num_cpus=self._num_cpus_worker, num_gpus=self._num_gpus_worker, memory=self._memory_worker)
