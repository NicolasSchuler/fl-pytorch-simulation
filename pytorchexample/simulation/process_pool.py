...
class ProcessPoolSimulator(Simulation):
    ...
    def run_loop(self):
        ...
        for server_round in range(1, self._fl_rounds + 1):
            ...
            fit_res, fit_failures = self._fit_clients(
                configure_fit_res, client_contexts
            )
            ...
            eval_res, eval_failures = self._evaluate_clients(
                configure_eval_res, client_contexts
            )
    ...
    def _evaluate_clients(self, configure_eval_res, client_contexts):
        ...
        return self._call_clients(
            configure_eval_res, client_contexts, ClientMethods.EVALUATE
        )
    ...
    def _call_clients(self, configure_res, contexts, method: ClientMethods):
        with tqdm(
            ...
            with ProcessPoolExecutor(
                ...
                tasks = {}
                cons = {
                    i: multiprocessing.Pipe(duplex=True)
                    for i in range(self._num_clients)
                }
                ...
                for c, ins in configure_res:
                    ...
                    if g_id not in dealt_with:
                        ...
                        padded_params = [
                            tensor + b"\x01" for tensor in ins.parameters.tensors
                        ]
                    ...
                    ins.parameters = Parameters(
                        tensors=[], tensor_type="padded.numpy.ndarray"
                    )
                    tasks[
                        p.submit(
                            fire_and_retrieve,
                    ...
                    for con in connection.wait(cons_parent):
                        ...
                        match method:
                            case ClientMethods.FIT | ClientMethods.GET_PARAMETERS:
                                name = msg[0]
                                padded_params_shm = shared_memory.ShareableList(
                                    name=name
                                )
                                p = Parameters(
                                    tensors=padded_params_shm,
                                    tensor_type="padded.numpy.ndarray",
                                )
